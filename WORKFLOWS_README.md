...
**✅ Fully centralized:** The asset generation workflow lives in `bgp-admin` and works on ANY app repository. No workflow files needed in individual app repos.

**How it works:**
1. bgp-admin uploads your source images (logo.png, splash.png, etc.) to the app's `assets/` folder via GitHub API
2. bgp-admin triggers the centralized workflow in `bgp-admin/.github/workflows/generate-assets.yml`
3. The workflow:
   - Clones the target app repository using GITHUB_PAT
   - Reads configuration from `assets/config.json` (background colors, etc.)
   - Runs `@capacitor/assets` to generate all platform-specific sizes
   - Commits and pushes the generated assets back to the app repository
4. ✅ Done! All sizes are generated and committed automatically

**What you need in your app repository:**
- ✅ An `assets/` directory with at least a `.gitkeep` file
- ✅ A valid `capacitor.config.ts` or `capacitor.config.json`
- ❌ NO workflow files needed!

**The centralized workflow is at:** `.github/workflows/generate-assets.yml` in bgp-admin
